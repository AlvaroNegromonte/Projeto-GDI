...
SELECT F.Nome
FROM Funcionarios F
WHERE (F.ID_Academia, F.Salario) = (SELECT F.ID_Academia, F.Salario FROM Funcionarios F
         WHERE F.CPF_Func ='11111111');


-- Subconsulta do tipo tabela -- Projetar o ID da academia e o CPF, nome e salário dos funcionários mais bem pagos de cada academia
SELECT ID_Academia, CPF_Func, Nome, Salario
FROM Funcionarios
WHERE (ID_Academia, Salario) IN (
    SELECT ID_Academia, MAX(Salario)
    FROM Funcionarios
    GROUP BY ID_Academia
); 
